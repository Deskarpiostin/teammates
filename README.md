...
```
git clone --recursive https://github.com/Deskarpiostin/teammates
```
...
```
cd C:\Users\username\projects\teammates
```

Note: if teammates is unpacked on another disk, nagivate there first:
```
D:
cd projects\teammates
```
...
```
`tm.dll` and `client.dll` will appear in the `build/dlls/Release` and `build/cl_dll/Release` directories.
...
`tm.dll` and `client.dll` will appear in `dlls/` and `cl_dll/` diretories. The libraries built with msvc6 should be compatible with Windows XP.
